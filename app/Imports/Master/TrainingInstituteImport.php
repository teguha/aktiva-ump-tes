...
class TrainingInstituteImport implements ToCollection, WithStartRow
{
    /**
    ...
    */
    public function collection(Collection $collection)
    {
        ...
        if (empty($row[1]) || strtoupper($row[1]) != strtoupper('Lembaga Training')) {
        ...
        foreach ($collection as $rw => $row) 
        {
            ...
            if (!empty($name)) {
                // Simpan Data
                TrainingInstitute::firstOrCreate(['name' => $name]);
            }
        }
