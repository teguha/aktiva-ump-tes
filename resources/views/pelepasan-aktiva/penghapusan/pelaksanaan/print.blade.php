...
<body class="page">
        ...
        <table style="border:none; width: 100%;">
            <tr>
                ...
                <td style="border:none;  text-align: center; font-size: 14pt;" width="auto">
                    <b>{{ __('PENGAJUAN PENGHAPUSAN AKTIVA') }}</b>
    ...
    <main>
        ...
        <br>
        <div style="text-align: justify;">Pengajuan penghapusan aktiva ini dibuat pada tanggal {{ $record->created_at->translatedFormat('d F Y')}} dengan pengajuan sebagai berikut:</div>
        <br>
        <ol>
            @foreach ($record->details()->dtGet()->get() as $detail)
                <li>
                    <table style="border: none; width: 100%">
                        <tr>
                            <td style="border: none;vertical-align: top; width: 170px;">{{ __('Kode Aktiva') }}</td>
                            <td style="border: none; vertical-align: top; width: 10px;">:</td>
                            <td style="border: none;">
                                <div class="wysiwyg-content">{{$detail->aktiva->code}}</div>
                            </td>
                        </tr>
                        <tr>
        ...
        <table style="border: none; ">
            <tr>
                <td style="border: none; width: 150px; vertical-align: top;">ID Pelaksanaan</td>
                <td style="border: none; width: 1em">:</td>
                <td style="border: none; ">
                    {{ $record->realization->code }}
                </td>
            </tr>
            <tr>
                <td style="border: none; vertical-align: top;">Tgl Pelaksanaan</td>
                <td style="border: none; width: 1em">:</td>
                <td style="border: none; ">
                    {{ $record->realization->date->format('d F Y') }}
                </td>
            </tr>
            <tr>
                <td style="border: none; vertical-align: top;">Keterangan</td>
                <td style="border: none; width: 1em">:</td>
                <td style="border: none; text-align: justify">
                    {{ $record->realization->description }}
                </td>
            </tr>
            <tr>
                <td style="border: none; vertical-align: top;">Lampiran</td>
                <td style="border: none; vertical-align: top; width: 1em">:</td>
                <td style="border: none; vertical-align: top;">
                    @foreach ($record->realization->files as $file)
                        <a href="{{ $file->file_url }}">{{ $file->file_name }}</a><br>
                    @endforeach
                </td>
            </tr>
        </table>
        <br>
        <div style="text-align: justify;">Demikian pengajuan penghapusan aktiva ini dibuat untuk dipertimbangkan sebaik-baiknya dan penuh tanggung jawab.</div>
        <br><br>
        @if ($record->realization->approval($module)->exists())
                            ...
                            <tr>
                                @if (!empty($ids[$i]))
                                    <td style="border: none; text-align: center; width: 33%; vertical-align: bottom;">
                                        @if ($approval = $record->realization->approval($module)->find($ids[$i]))
                                ...
                                @if (!empty($ids[$i + 1]))
                                    <td style="border: none; text-align: center; width: 33%; vertical-align: bottom;">
                                        @if ($approval = $record->realization->approval($module)->find($ids[$i + 1]))
                                            @if ($approval->status == 'approved')
                                                <div style="height: 110px; padding-top: 15px;">
                                                {!! \Base::getQrcode('Approved by: ' . $approval->user->name . ', ' . $approval->approved_at) !!}
                                                </div>
                                ...
                                @if (!empty($ids[$i + 2]))
                                    <td style="border: none; text-align: center; width: 33%; vertical-align: bottom;">
                                        @if ($approval = $record->realization->approval($module)->find($ids[$i + 2]))
                                            @if ($approval->status == 'approved')
                                                <div style="height: 110px; padding-top: 15px;">
                                                {!! \Base::getQrcode('Approved by: ' . $approval->user->name . ', ' . $approval->approved_at) !!}
                                                </div>
