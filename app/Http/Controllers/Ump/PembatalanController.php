...
use App\Http\Requests\Ump\PembatalanUmpRequest;
...
{
    protected $module = 'ump.pembatalan-ump';
    protected $routes = 'ump.pembatalan-ump';
    protected $views = 'ump.pembatalan';
    protected $perms = 'ump.pembatalan-ump';
    ...
    public function __construct()
    {
        $this->prepare([
            ...
            'title' => 'Pembatalan UMP',
            'breadcrumb' => [
                'Home' => route('home'),
                'UMP' => '#',
                'Pembatalan UMP' => route($this->routes.'.index'),
            ]
        ]);
    }
        ...
        $this->prepare([
            'tableStruct' => [
                'datatable_1' => [
                    ...
                    $this->makeColumn('name:pembatalan|label:Pembatalan|className:text-center'),
    ...
    {
        $user = auth()->user();
        $records = PembatalanUmp::with('pengajuanUmp', 'pengajuanUmp.pengajuanSgu', 'pengajuanUmp.aktiva')->grid()->filters()->dtGet();
        ...
        return \DataTables::of($records)
            ...
            ->addColumn('pembatalan', function ($record) {
                if($record->id_ump_pembatalan){
                    return $record->id_ump_pembatalan . "<br>" . $record->tgl_ump_pembatalan->format('d/m/Y');
                }
                return "-";
            })
            ...
            ->rawColumns(['action','updated_by','status', 'code', 'pembatalan'])
            ->make(true);
    }
        ...
        $this->prepare([
            'tableStruct' => [
                ...
                'datatable_2' => [
                    ...
                    $this->makeColumn('name:id_pembatalan|label:ID Pembatalan UMP|className:text-center'),
                    $this->makeColumn('name:status'),
                    $this->makeColumn('name:updated_by'),
                    $this->makeColumn('name:action'),
                ],
    ...
    public function gridDetail()
    {
        ...
        return \DataTables::of($records)
            ...
            ->addColumn('nominal', function ($record) {
             ...
             ->addColumn('id_pembatalan', function ($record) use ($user) {
                return $record->pembatalanUmp ? $record->pembatalanUmp->id_pembatalan : '';
            })
            ->addColumn('status', function ($record) use ($user) {
                return $record->pembatalanUmp ? $record->pembatalanUmp->status : '';
            })
            ->addColumn('updated_by', function ($record) {
                if($record->pembatalanUmp){
                    return $record->pembatalanUmp->createdByRaw();
                }
                return '';
            })
            ->addColumn('action', function ($record) use ($user) {
    ...
    public function extend(PjUmp $record)
    {
        ...
        $page_action = 'create';
        $pj = $record;
        $record = new PembatalanUmp();
        return $this->render($this->views.'.create', compact('rekening', 'record', 'pj', 'page_action', 'pic', 'kepala_dept'));
    }

    public function store(PembatalanUmpRequest $request)
    {
        $record = new PembatalanUmp();
        return $record->handleStoreOrUpdate($request);
    }

    public function edit(PembatalanUmp $record)
    {
        return $this->render($this->views.'.edit', compact('record'));
    }

    public function update(PembatalanUmp $record, PembatalanUmpRequest $request)
    {
        return $record->handleStoreOrUpdate($request);
    }

    public function show(PembatalanUmp $record)
    {
        return $this->render($this->views.'.show', compact('record'));
    }

    public function approval(PembatalanUmp $record)
    {
        return $this->render($this->views.'.approval', compact('record'));
    }

    public function approve(PembatalanUmp $record, Request $request)
    {
        return $record->handleApprove($request);
    }

    public function cancel(PembatalanUmp $record, Request $request)
    {
        return $record->handleCancel($request);
    }

    public function revise(PembatalanUmp $record, Request $request)
    {
        return $record->handleRevise($request);
    }

    public function reject(PembatalanUmp $record, Request $request)
    {
        return $record->handleReject($request);
    }

    public function verification(PembatalanUmp $record)
    {
        $pic = $record->pengajuanUmp()->picStaff;
        $kepala_dept = $record->pengajuanUmp()->picKepala;
        $rekening =  RekeningBank::where('user_id', $pic->id)->first();
    ...
    public function verify(PembatalanUmp $record, Request $request)
    {
        return $record->handleVerify($request);
    }

    public function history(PembatalanUmp $record)
    {
        $this->prepare(['title' => 'History Aktivitas']);
        return $this->render('globals.history', compact('record'));
    }

    public function tracking(PembatalanUmp $record)
    {
        return $this->render('globals.tracking', compact('record'));
    }


    public function print(PembatalanUmp $record)
    {
