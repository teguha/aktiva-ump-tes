<div class="col-md-6">
	<div class="card card-custom card-stretch gutter-b chart-pelaporan-wrapper">
		<div class="card-header h-auto py-3">
			<div class="card-title">
				<h3 class="card-label">
					<span class="d-block text-dark font-weight-bolder">{{ __('Termin Pembayaran') }}</span>
				</h3>
			</div>
			<div class="card-toolbar" style="max-width: 500px;">
				<form id="filter-chart-pelaporan"
					action="{{ route($routes.'.chartTermin') }}"
					class="form-inline"
					role="form">
...
@push('scripts')
	<script>
		$(function() {
		    iniFilterchartTermin();
		    drawchartTermin();
		});

		var iniFilterchartTermin = function () {
		...
		var drawchartTermin = function () {
			var filter = $('#filter-chart-pelaporan');
			...
			$.ajax({
				...
				success: function (resp) {
					$('.chart-pelaporan-wrapper .chart-wrapper').find('#chart-pelaporan').remove();
					$('.chart-pelaporan-wrapper .chart-wrapper').html(`<div id="chart-pelaporan"></div>`);
					renderchartTermin(resp);
				},
				error: function (resp) {
					console.log(resp)
				}
		...
		var renderchartTermin = function (options = {}) {
			var element = document.getElementById('chart-pelaporan');
