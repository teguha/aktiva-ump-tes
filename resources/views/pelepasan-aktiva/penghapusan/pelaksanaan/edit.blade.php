...
@section('action', route($routes . '.update', $penghapusan->id))
...
@section('page-content')
    ...
    <div class="row mb-3">
        ...
        <div class="card-body">
            @csrf
            @include('pelepasan-aktiva.penghapusan.subs.card-1', ['record'=>$penghapusan])
        </div>
    </div>
</div>
</div>
...
<div class="col-sm-12">
    <div class="card card-custom">
        ...
        <div class="card-body p-8">
            @csrf
            @include('pelepasan-aktiva.penghapusan.subs.card-2', ['record'=>$penghapusan])
        </div>
            ...
            <div class="card-body p-8">
                <div class="form-group row">
                    ...
                    <div class="col-4 parent-group">
                        <input type="hidden" name="pengajuan_id" value="{{ $penghapusan->id }}">
                        <input name="code" class="form-control" placeholder="{{ __('ID Pelaksanaan') }}" value="{{ $penghapusan->realization->code??'' }}">
                    </div>
                    ...
                    <div class="col-4 parent-group">
                        <input name="date" class="form-control base-plugin--datepicker" placeholder="{{ __('Tgl Pelaksanaan') }}"
                            value="{{ isset($penghapusan->realization->date) ? $penghapusan->realization->date->format('d/m/Y') :'' }}">
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-2 col-form-label">{{ __('Keterangan') }}</label>
                    <div class="col-10 parent-group">
                        <textarea name="description" class="form-control" placeholder="{{ __('Keterangan') }}">{!! $penghapusan->realization->description??'' !!}</textarea>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-10 parent-group">
                        <div class="custom-file">
                            ...
                            <input type="file" class="custom-file-input base-form--save-temp-files" data-name="uploads"
                                data-container="parent-group" data-max-size="2048" data-max-file="10" accept="*" multiple>
                            <label class="custom-file-label" for="file">Choose File</label>
                            @foreach (($penghapusan->realization->files ?? []) as $file)
