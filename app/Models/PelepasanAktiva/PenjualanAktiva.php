...
class PenjualanAktiva extends Model
{
    use HasFiles, HasApprovals;

    protected $table = 'trans_penjualan_aktiva';
    ...
    {
        return $this->hasMany(PenjualanAktivaDetail::class, 'pengajuan_id');
    }

    function realization()
    {
        return $this->hasOne(PelaksanaanPenjualanAktiva::class, 'penjualan_aktiva_id');
    }
    ...
    {
        return $query->filterBy(['pengajuan_id', 'status', 'struct_id']);
    }
    ...
    public function handleDetailStoreOrUpdate($request)
    {
        $this->beginTransaction();
        try {
            $detail = PenjualanAktivaDetail::firstOrNew([
    ...
    public function handleDetailDestroy(PenjualanAktivaDetail $detail)
    {
        $this->beginTransaction();
        try {
            $this->saveLogNotify();
