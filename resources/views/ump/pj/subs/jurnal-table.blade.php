@section('dataFilters')
    ...
    @if(isset($jurnalTableStruct['datatable_2']))
        <table id="datatable_2" class="table table-bordered table-hover is-datatable" style="width: 100%;" data-url="{{ isset($jurnalTableStruct['url']) ? $jurnalTableStruct['url'] : route($routes.'.grid') }}" data-paging="{{ $paging ?? true }}" data-info="{{ $info ?? true }}">
            <thead>
                <tr>
                    @foreach ($jurnalTableStruct['datatable_2'] as $struct)
