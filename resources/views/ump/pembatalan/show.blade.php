            ...
            <div class="card card-custom">
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-12 col-md-6">
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-4 col-form-label">{{ __('No Pembatalan UMP') }}</label>
                                <div class="col-sm-12 col-md-8 parent-group">
                                    <input type="text" name="id_ump_pembatalan" class="form-control" placeholder="{{ __('No Pembatalan UMP') }}"
                                    value="{{ $record->id_ump_pembatalan}}" disabled>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-4 col-form-label">{{ __('Tgl  Pembatalan UMP') }}</label>
                                <div class="col-sm-12 col-md-8 parent-group">
                                    <input type="text" name="tgl_ump_pembatalan" class="form-control base-plugin--datepicker"
                                        data-options='@json([
                                            "endDate" => now()->format("d/m/Y") ])',
                                        placeholder="{{ __('Tgl Pembatalan UMP') }}"
                                        value="{{ $record->tgl_ump_pembatalan ?  $record->tgl_ump_pembatalan->format('d/m/Y') : '' }}" disabled>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-12">
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-2  col-form-label">{{ __('Lampiran Pembatalan UMP') }}</label>
                                <div class="col-sm-12 col-md-10  parent-group">
                                    @foreach ($record->files('ump.pembatalan-ump')->where('flag', 'uploads')->get() as $file)
                                    <div class="progress-container w-100" data-uid="{{ $file->id }}">
                                        <div class="alert alert-custom alert-light fade show py-2 px-3 mb-0 mt-2 success-uploaded" role="alert">
                                            <div class="alert-icon">
                                                <i class="{{ $file->file_icon }}"></i>
