...
{
    protected $module = 'master_org_branch';
    ...
    public function __construct()
    {
        $this->prepare([
            ...
            'breadcrumb' => [
                ...
                'Regional' => route($this->routes.'.index'),
            ]
        ]);
    }
    ...
    {
        $user = auth()->user();
        $records = OrgStruct::branch()->filters()->dtGet();
            ...
            ->addColumn('action', function ($record) use ($user) {
                ...
                if ($record->canDeleted()) {
                    $actions[] = [
                        'type' => 'delete',
                        'id' => $record->id,
                        'attrs' => 'data-confirm-text="' . __('Hapus parameter Regional') . ' ' . $record->name . '?"',
                    ];
                }
    ...
    public function update(BranchRequest $request, OrgStruct $record)
    {
        return $record->handleStoreOrUpdate($request, 'branch');
    }
