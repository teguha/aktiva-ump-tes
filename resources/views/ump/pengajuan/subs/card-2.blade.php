@php
    ...
    $location = $position ? $position->location : null;
@endphp
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-md-4 parent-group">
                <select class="form-control base-plugin--select2-ajax" name="struct"
                    data-url="{{ route('ajax.selectStruct', 'all') }}" data-placeholder="{{ __('Pilih Salah Satu') }}"
                    disabled>
                    <option value="{{ $record->struct ?? ($location ? $location->id : '') }}" selected
                        {{ in_array($page_action, ['show', 'approval']) }}>
                        {{ $record->struct ? $record->struct->name : ($location ? $location->name : '') }}
                    </option>
                </select>
                <input type="hidden" name="struct" value="{{ $record->struct ?? ($location ? $location->id : '') }}">
            </div>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="col-sm-12 col-md-12">
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-md-4 parent-group">
                <input type="text" name="kepala_dept_nik" class="form-control" placeholder="{{ __('NIK') }}"
                    value="{{ $kepala_dept->nik ?? '' }}" disabled>
            </div>
            <div class="col-sm-12 col-md-4 parent-group">
                <input type="text" name="kepala_dept_username" class="form-control"
                    placeholder="{{ __('Nama') }}" value="{{ $kepala_dept->username ?? '' }}" disabled>
            </div>
